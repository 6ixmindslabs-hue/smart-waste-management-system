...
const StatusCard = ({ data }) => {
    ...
    return (
        <article className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:border-cyan-200 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-cyan-900/70">
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    <p className="text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">{t('deviceId')}</p>
                    <h3 className="mt-1 truncate text-lg font-bold text-slate-950 dark:text-white" title={friendlyName || deviceId}>
                ...
                <div className={`flex shrink-0 items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs font-bold uppercase ${current.bg} ${current.text} ${current.border}`}>
